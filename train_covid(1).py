...
import gc
import itertools
from config_covid import Config
import torch
import scanpy as sc
import pandas as pd
...
class Train(Config):
    ...
    def run(self):
        sex_mapping = {}
        for i, k in enumerate(set(self.adata1.obs['Cell_type_annotation_level2'])):
            sex_mapping[k] = i
        clusters_true1 = self.adata1.obs['Cell_type_annotation_level2']
        clusters_true1 = clusters_true1.map(sex_mapping)
        clusters_true2 = self.adata2.obs['Cell_type_annotation_level2']
        clusters_true2 = clusters_true2.map(sex_mapping)
        3#print(isnan(clusters_true2.any()))
