...
from unifan.KLCLR import KLCLR
...
class Train(Config):
    ...
    def leiden_cluster(self, z_init):
        ...
        sc.tl.leiden(adata, random_state=self.random_seed)
    ...
    def run(self):
        ...
        if os.path.isfile(self.input_z_ZINB_path) and os.path.isfile(self.input_ae_ZINB_path):
            ...
            trainer = Trainer(dataset = self.expression_integrated, model=model_autoencoder, model_name="pretrain_z_ZINB", batch_size=self.batch_size,
        ...
        clusters_pre1, centroids_torch_project1 = self.leiden_cluster(z_init1)
        clusters_pre2, centroids_torch_project2 = self.leiden_cluster(z_init2)

        # ------ Define the Contrastive dataset ------
        subjects = np.hstack((np.zeros(self.N_project1), np.ones(self.N_project1)))
        Contr_expression = ContrDataset(self.x, np.zeros(self.N), subjects)   

        for iteration in range(1, self.num_iteration+1):
            annocluster_ae_ZINB_path = os.path.join(self.annocluster_ZINB_folder, f"anno_ZINB_model_optimal_{iteration}.pickle")
            ...
            trainer = Trainer(dataset=Contr_expression, model=model_annocluster,
            ...
            if os.path.isfile(annocluster_ae_ZINB_path):
            ...
            use_pretrain = True
            constractive_ae_ZINB_path = os.path.join(self.model_contrastive_folder, f"constractive_model_optimal_{iteration}.pickle")
            ...
            trainer_contrastive = Trainer(dataset=Contr_expression, model=model_contrastive,
            ...
            if os.path.isfile(constractive_ae_ZINB_path):
